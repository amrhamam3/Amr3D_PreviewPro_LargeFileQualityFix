...
class DXF2DView @JvmOverloads constructor(
    ...
    private fun animatePointRadius(grow: Boolean) {
        ...
        val targetLarge = (basePointRadiusDp + 6f) * density
    ...
    private val measureTextPaint = Paint().apply {
        color = Color.parseColor("#FF8A1E")
        textSize = measureTextSizeSp * scaledDensity
        isAntiAlias = true
        isFakeBoldText = true
    }
            ...
            when (event.actionMasked) {
                MotionEvent.ACTION_DOWN -> {
                    ...
                    if (hitPoint!= null) {
                        ...
                        dragLiveWorld = if (hitPoint.second == 0) seg.p1 else seg.p2
                        return true
                    }
                }
                MotionEvent.ACTION_MOVE -> {
                    draggingPoint?.let { (segIdx, pointIdx) ->
                        val newWorld = resolveWorldPoint(event.x, event.y)
                        dragLiveWorld = newWorld
    ...
    private fun resolveWorldPoint(screenX: Float, screenY: Float): FloatArray {
        ...
        return floatArrayOf(screenToWorldX(screenX), screenToWorldY(screenY))
    }
    ...
    private fun buildRenderCache() {
        ...
        fun addSegment(color: Int, x1: Float, y1: Float, x2: Float, y2: Float) {
            buckets.getOrPut(color) { ArrayList() }.apply { add(x1); add(y1); add(x2); add(y2) }
        }
        for (line in m.lines) {
    ...
    private fun buildSnapPoints(m: DxfModel): List<FloatArray> {
        ...
        for (circle in m.circles) {
            ...
            pts.add(floatArrayOf(circle.cx, circle.cy))
        }
        for (arc in m.arcs) {
            if (!isLayerVisible(arc.layer)) continue
            pts.add(floatArrayOf(arc.cx, arc.cy))
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        for (circle in m.circles) {
            ...
            canvas.drawCircle(toScreenX(circle.cx), toScreenY(circle.cy), circle.r * scale, defaultPaint)
        }
        drawGapHighlight(canvas)
        drawMeasurement(canvas) // 1. فوق الكل
    }
    ...
    private fun drawMeasurement(canvas: Canvas) {
        ...
        for ((segIndex, seg) in measureSegments.withIndex()) {
            ...
            val isDraggingP1 = draggingPoint == Pair(segIndex, 0)
            val isDraggingP2 = draggingPoint == Pair(segIndex, 1)
        ...
        tempP1?.let { p1 ->
            ...
            if (isDragPlacing) {
                dragLiveWorld?.let { live ->
                    ...
                    val angle = atan2((syLive - sy1).toDouble(), (sxLive - sx1).toDouble()).toFloat()
                    drawPinPoint(canvas, sx1, sy1, angle + Math.PI.toFloat(), false)
                    val distMm = hypot((live[0] - p1[0]).toDouble(), (live[1] - p1[1]).toDouble()).toFloat()
                    drawMeasureLine(canvas, sx1, sy1, sxLive, syLive, distMm, angle, drawDeleteButton = false)
                }
            } else {
                drawPinPoint(canvas, sx1, sy1, 0f, false)
            }
        }

        if (isDragPlacing || draggingPoint!= null) {
    ...
    private fun drawPinPoint(canvas: Canvas, cx: Float, cy: Float, lineAngleRad: Float, isDragging: Boolean) {
        ...
        val r = if (isDragging) pointRadiusPx * 1.8f else pointRadiusPx * 1.3f // 2. بيكبر
        val pinTilt = Math.toRadians(-15.0).toFloat()

        canvas.save()
        canvas.rotate(Math.toDegrees((lineAngleRad + pinTilt).toDouble()).toFloat(), cx, cy)

        canvas.drawCircle(cx, cy - r * 0.35f, r * 0.55f, paintStroke)
        canvas.drawCircle(cx, cy - r * 0.35f, r * 0.25f, paintFill)

        val path = Path()
        path.moveTo(cx - r * 0.18f, cy - r * 0.35f)
        path.lineTo(cx + r * 0.18f, cy - r * 0.35f)
        path.lineTo(cx, cy + r * 0.85f)
        path.close()
    ...
    private fun drawMeasureLine(canvas: Canvas, sx1: Float, sy1: Float, sx2: Float, sy2: Float, distMm: Float, lineAngleRad: Float, drawDeleteButton: Boolean): FloatArray {
        ...
        canvas.save() // 3. النص بيلف
        canvas.rotate(Math.toDegrees(lineAngleRad.toDouble()).toFloat(), labelX, labelY)
    ...
    private fun drawMagnifier(canvas: Canvas, worldX: Float, worldY: Float) {
        ...
        val extraTopOffset = 40f * density

        val magCenterX = margin + radius
        val magCenterY = margin + radius + extraTopOffset

        val zoom = 3.5f
        val magScale = scale * zoom
        canvas.save()
        val clipPath = Path().apply { addCircle(magCenterX, magCenterY, radius, Path.Direction.CW) }
        canvas.clipPath(clipPath)
        canvas.drawCircle(magCenterX, magCenterY, radius, bgPaint)
        for ((color, modelCoords) in lineColorGroups) {
        ...
        model?.circles?.forEach { c ->
            ...
            canvas.drawCircle(magCenterX + (c.cx - worldX) * magScale, magCenterY - (c.cy - worldY) * magScale, c.r * magScale, magnifierLinePaint)
        }
        canvas.restore()
